...
exports.query = function (req, res) {
  doSync(function querySync() {
    ...
    //CUSTOM CODE - code for geo queries
    var latitude = req.param('latitude');
    var longitude = req.param('longitude');
    var radius = req.param('radius');

    if (_exists(latitude) && _exists(longitude)) {
      if (_exists(radius)) {
        //query = { 'loc': {$geoWithin: { $centerSphere: [[parseFloat(longitude), parseFloat(latitude)], parseFloat(radius) / 3959]}}};
        query.loc = {$geoWithin: { $centerSphere: [[parseFloat(longitude), parseFloat(latitude)], parseFloat(radius) / 3959]}};
      } else {
        //query = { 'loc': {$geoWithin: { $centerSphere: [[parseFloat(longitude), parseFloat(latitude)], 30 / 3959]}}};
        query.loc = {$geoWithin: { $centerSphere: [[parseFloat(longitude), parseFloat(latitude)], 30 / 3959]}};
      }
    }
